...
///   HIGH-2  Shares mint against the registry mark but redeem at the live pool
///           price; a mark/pool divergence during a freeze is an arb against
...
///           (Mitigated operationally by TWAP marks + deep-pool curation.)
...
contract StockTreasuryAuditFindingsTest is Test {
    ...
    function test_HIGH1_ReturnTokensSellHonorsSlippageFloor() public {
        ...
        // marked value. The mark notional (~$1000) still clears the early
        // gross-notional check, so only a realized-output floor can catch it.
    ...
    function test_HIGH1_ReturnTokensInKindStillAllowedBelowFloor() public {
        ...
        // floor is intentionally not enforced (mark-notional floor governs).
    ...
    // ─── HIGH-3 ─────────────────────────────────────────────────────────────
    // A held leg that becomes unpriceable makes registry.valueOf revert, which
    // pre-fix reverted heldTokenValues()/withdrawLtsTo() entirely — trapping
    // every holder, even with returnTokens=true. Post-fix the unpriceable leg is
    // settled in-kind so the holder always exits.
    function test_HIGH3_UnpriceableLegDoesNotTrapHolders() public {
        (StockTreasury treasury, AgentCurve curve) = _build(_pair(), 1_000 * 1e6, 0, new uint256[](2));
        uint256 aliceShares = curve.balanceOf(alice);

        // AAPL becomes unpriceable (dead pool / delisting); keep TSLA priceable.
        ...
        // in-kind. Pre-fix this whole call reverts.
        vm.prank(alice);
