...
import {EquityTreasury} from "./EquityTreasury.sol";

/// @title EquityTreasuryFactory
/// @notice Deploys {EquityTreasury} beacon proxies (and their spawned
/// {AgentCurve}) for real tokenized-equity baskets. Byte-for-byte the same
/// shape as {StockTreasuryFactory}; the ONLY substantive difference is that it
/// wires the forked Uniswap v4 `ROUTER` (rather than the v3 `SWAP_ROUTER`) into
/// each treasury via the `ROUTER()` getter the treasury reads at init.
///
/// Deploy this factory ALONGSIDE {StockTreasuryFactory} — the two coexist, one
/// per asset class (equity vs memecoin). They share nothing at runtime.
contract EquityTreasuryFactory is Ownable2Step, ReentrancyGuard {
    using SafeERC20 for IERC20;

    IERC20 public immutable STABLE;
    /// The forked Uniswap v4 UniversalRouter.
    ...
    struct DeployParams {
        ...
        address rebalancer;
        EquityTreasury.AssetSpec[] initialPortfolio;
        string reasoningCid;
        string curveName;
        string curveSymbol;
    ...
    /// withdrawLtsTo. Halts buys and sells across all equity treasuries at once.
    bool public paused;
    ...
    constructor(
        ...
        if (
            stable_ == address(0) || router_ == address(0) || registry_ == address(0)
    ...
    /// @notice Deploy a BeaconProxy fronting an {EquityTreasury} (and its
    /// spawned {AgentCurve}) funded by user permit.
        ...
        returns (address treasury, address curve)
    {
        ...
        curve = EquityTreasury(treasury).curve();
    ...
    /// @notice Owner-only emergency stop halting buys and sells across every
    /// equity treasury this factory deployed.
    ...
    function _initData(DeployParams calldata p) internal view returns (bytes memory) {
        return abi.encodeCall(
            ...
            (p.rebalancer, p.user, address(this), p.initialPortfolio, p.reasoningCid, _curveInit(p))
        );
    }

    function _proxyInitCode(bytes memory initData) internal view returns (bytes memory) {
        return abi.encodePacked(type(BeaconProxy).creationCode, abi.encode(TREASURY_BEACON, initData));
    }

    function _curveInit(DeployParams calldata p) internal view returns (EquityTreasury.CurveInitParams memory) {
        return EquityTreasury.CurveInitParams({
            name: p.curveName,
